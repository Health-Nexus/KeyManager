...
export class DrsService {
  ...
  */
  ...
  /**
   * dataRequestTest function.  Sends signed message to a gatekeeper, 
   * based on key, parameter and key url, to retrive data.
  ...
  dataRequestTest(urlKey, parameter, key): any {
    ...
    let signature;
    let signer = this.unlockedAccount || 
   ...
   * @return {json} error or success.  Success contains a boolean of if 
   * its a owner or not
   */
  ...
  permissionKey(id, canShare, canTrade, canSell): any {
    let p = new Promise<any>((resolve, reject) => {
      this._contract.at(this.contractAddr).setKeyPermissions(
          id, 
          canShare, 
          canTrade, 
          canSell, 
    ...
    if (typeof window['web3'] !== 'undefined' && (!localStorage['nodeIP'] || 
      ...
      this.handleConnection(this.web3.isConnected());
        ...
        this.web3 = new this.Web3(
            new this.Web3.providers.HttpProvider(this.nodeIP)
        );
        this.handleConnection(this.web3.isConnected());
      };
  };
